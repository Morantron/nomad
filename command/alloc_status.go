...
func (c *AllocStatusCommand) Run(args []string) int {
	...
	if short {
		...
		if checkOutput := formatAllocNomadServiceChecks(alloc, client); checkOutput != "" {
			c.Ui.Output("")
...
func formatAllocNomadServiceChecks(alloc *api.Allocation, client *api.Client) string {
	statuses, err := client.Allocations().Checks(alloc, nil)
	if err != nil {
		return ""
